            ...
            var divNumber = 0;
             for (var i = 0; i < results.length; i++) {
                ...
                if (results[i].img_link == "" || !results[i].img_link) {
                 ...
                 htmlString = htmlString + '<div class="col-md-4 d-flex"><div class="blog-entry justify-content-end"><a class="block-20" style="background-image: url(\'' + image + '\');"></a ><div class="text p-4 float-right d-block"><h3 class="heading mb-0"><a>' + plantName + '</a></h3><p>' + "Flower Color: " + results[i].flower_color + '<br />' + "Height: " + results[i].min_height + " - " + results[i].max_height + 'm<br />' + "Season:" + results[i].flower_time + '<br />' + "Soil Texture: " + results[i].soil_texture + fill+'</p><p><a id="addButton" href="/clicked" class="btn btn-primary" style="font-size:12px">Add to Collection</a></p></div></div></div>';
