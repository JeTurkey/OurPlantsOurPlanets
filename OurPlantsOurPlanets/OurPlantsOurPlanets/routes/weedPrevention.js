...
router.get('/', function (req, res) {
    ...
    fs.readFile(path.join('D:/home/site/wwwroot' + '/views/header1.html'), function (err, data) {
        ...
        fs.readFile(path.join('D:/home/site/wwwroot' + '/views/weedEncyclopedia.html'), function (err, data) {
            ...
            //query for predictions in next three months
            ...
            connection.query(queryString, function (error, results, fields) {
                ...
                for (var i = 0; i < results.length; i++) {
                    ...
                    //implement cards and pagination
                ...
                connection.query(queryString, function (error, results, fields) {
                    ...
                    if (results.length > 6) {
                        ...
                        for (var j = 0; j < Math.ceil(results.length / 6); j++) {
                            ...
                            htmlString = htmlString + '<button style="background-color:Transparent;border:none" class="number-name" value="' + (j + 1) + 'n" id="' + (j + 1) + 'nbutton" onClick="paginateWeed(this.value)" >' + (j + 1);
                            htmlString = htmlString + '</button></div>';
                        }
                        htmlString = htmlString + '</div></div></section>';
                    }
                    ...
                    connection.query(queryString, function (error, results, fields) {
                        ...
                        htmlString = htmlString + '<section class="ftco-section bg-light collapse" id="collapseThree"><div class="container"><div class="row d-flex">';
                        var divNumber = 0;
                        for (var i = 0; i < results.length; i++) {
                            if (Math.ceil((i + 1) / 6) > divNumber) {
                                divNumber = divNumber + 1;
                        ...
                        if (results.length > 6) {
                            ...
                            for (var j = 0; j < Math.ceil(results.length / 6); j++) {
                                ...
                                htmlString = htmlString + '<button style="background-color:Transparent;border:none" class="number-name" value="' + (j + 1) + 'nn" id="' + (j + 1) + 'nnbutton" onClick="paginateWeed(this.value)" >' + (j + 1);
                                htmlString = htmlString + '</button></div>';
                            }
                            htmlString = htmlString + '</div></div></section></section>';
                        }
