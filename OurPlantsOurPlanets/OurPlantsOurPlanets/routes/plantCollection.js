...
router.get('/', function (req, res) {
    ...
    fs.readFile(path.join('D:/home/site/wwwroot' + '/views/header1.html'), function (err, data) {
        ...
        fs.readFile(path.join('D:/home/site/wwwroot' + '/views/weedEncyclopedia.html'), function (err, data) {
            htmlString = htmlString + data.toString().substr(1).replace("Weed Encyclopaedia", "Garden Planner").replace("bg-weedcontrol", "IMG-20200518-WA0003 (1)");
            if (!req.session.acookie) {
                ...
                connection.query(queryString, function (error, results, fields) {
                    ...
                    htmlString = htmlString + '<table style="width:100%;text-align:center;margin-left:20px;border:1px solid #4e9525"><tr><th style="text-align:center;background-color:#4e9525;color:white;padding:8px">Garden Overview</th></tr><tr>';
                    htmlString = htmlString + "<td><b>Height:</b><br/> " + minheight + " - " + maxheight + "meters</td></tr>";
                    htmlString = htmlString + "<tr><td><b>Soil Textures:</b><br/> " + textureString.substr(0, textureString.length - 1) + "</td></tr>";
                    ...
                    for (var t = 0; t < results.length; t++) {
                        ...
                        htmlString = htmlString + '<td>' + '<form autocomplete="off" action="/plantCollection?id=' + results[t].common_name +'" method="POST"><input type="submit" value="Remove" class="btn btn-primary" style="font-size:medium;margin-top:5px"></form>' + '</td>';
                        htmlString = htmlString + '</tr>';
                    }
                    htmlString = htmlString + "</table></section>";
